...
\caption{Prior imposed on $\epsilon$ by assuming a uniform prior on $\hat{A}$ and $\chi$. Equivalently, the Jacobian in transforming a probability densitDistribution imposed on the overall mode amplitude $A$ of Eq.~\eqref{eq:ellip} by assuming a uniform prior on the circular mode amplitudes $A_R, A_L$ in the range $[0, 1]$.}y from $\{\hat{A},\chi\}$ to $\{A,\epsilon\}$, \eq{jac_Aeps_Achi}.}
