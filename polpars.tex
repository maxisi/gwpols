...
In full analogy to electromagnetic waves, a GW is elliptically polarized if there exists \emph{some} choice of $\hat{x}$ such that the two polarizations can be written as
\beq
h_+ = A(t) \cos \Phi(t) \, ,
\eeq
\beq
...
In the case of the inspiral signal above, it is immediately clear that Eq.~\eqref{eq:cbc_p} .The ellipticity is simply the ratio of plus and cross amplitudes in this frame \mi{check sign}
\beq
...
Crucially, this does not mean that an elliptical wave will take this form for \emph{any} choice of $\hat{h}$; in fact, it will not unless $\epsilon=\pm1$
