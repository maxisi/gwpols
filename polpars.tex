...
However, if $\theta$ and $\phi$ are allowed to very independently over this range, this results in a double covering of the waveform phase space, since the template is invariant under the addition or subtraction of $\pi$ to both $\theta$ and $\phi$, i.e., the transformations $\{\theta, \phi\} \to \{\theta \pm \pi, \phi \pm \pi\}$ for any combination of plus and minus signs; this degeneracy is easy to infer from Fig.~\ref{fig:ellipse}, and can be traced back to the property discussed in Footnote \ref{foot:angles} in relation to $\phi_{R/L}$.
