...
For a given $\hat{k}$, it is straightforward to check that these tensors are orthogonal such that $e^p_{ij} e^{p'ij}=2\delta^{pp'}$ for $p,p'$ in $\{+,\times\}$.
...
\end{align}
where the sum is over linear polarization states defined in some wave frame attached to the propagation direction $\hat{k}$,%
...
If this quantity, the \emph{complex strain}, is purely real (imaginary),
...
This becomes obvious if we note that, by Eq.~\eqref{eq:circ_amps} and the definition of $\tilde{H}$, it is the case that $\tilde{H}(\omega) = \sqrt{2}\, \tilde{h}_R (\omega)$ and so, rewriting Eq.~\eqref{eq:hcomp_fd} as an integral restricted to positive frequencies,
...
\end{align}
where
$A \equiv \sqrt{2} \left(|\tilde{h}_R| + |\tilde{h}_L| \right)$ is the overall amplitude of the Fourier mode ($A \geq 0$), $\epsilon = (|\tilde{h}_R| - |\tilde{h}_L|)/(|\tilde{h}_R| + |\tilde{h}_L|)$ is its ellipticity ($-1 \leq \epsilon \leq 1$), and $\phi_{R/L} = \mathrm{arg}(\tilde{h}_{R/L})$ are the right- and left-handed Fourier phases.
With some trigonometry, it is easy to show that this corresponds to linear polarization functions given by
