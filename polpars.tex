...
With that constraint, the $\epsilon$ distribution (inset histogram) is inversely proportional to the Jacobian in \eq{jac_Aeps_Achi}: a uniform prior on $\hat{A}$ and $\chi$ implicitly favors linear polarizations ($\epsilon = 0$) over circular ones ($\epsilon = \pm 1$), with probability density $\propto 1/J_0$ (orange curve). }
...
Based on \eq{Cphi_to_Aellip}, this is also proportional to $1/A$.
...
\caption{Distribution imposed on the overall mode amplitude $A$ of Eq.~\eqref{eq:hcomp_ellip} by assuming a uniform prior on the circular mode amplitudes $A_R, A_L$ in the range $[0, 1]$.}
\label{fig:jac_rl}
\end{figure}
...
Accordingly, the Jacobian that takes us from the circular parametrization to one flat in $\{\hat{A},\chi\}$ can be shown to be $J = \hat{A}^{-1}/2$.
