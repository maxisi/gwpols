...
We call this the \emph{wave frame}.
...
\caption{Polarization ellipse. At any given time, the phasor of an elliptically polarized signal lies on an ellipse with some amplitude $A(t)$ and ellipticity $\epsilon$, with semimajor axis tilted by an angle $\theta$ with respect to the plus-polarization axis (abscissa); a second angle, $\phi \equiv \Phi(t=0)$, determines the initial location of the phasor within the ellipse. (Adapted from \cite{Isi:2021iql}.)}
\label{fig:ellipse}
\end{figure}
...
When working with a single elliptically-polarized wave, Eq.~\eqref{eq:ellip_frame} defines a natural orientation of the wave frame, unique up to $\pi$ rotations.
If we adopt $\theta\hspace{-4pt} \mod{\pi /2} =0$ as a convention (or, equivalently, $\theta=\pi$), then the polarization angle $\psi$ becomes synonymous with the polarization ellipse orientation.
However, the two angles are conceptually distinct in general.


\subsubsection*{Nonprecessing compact binaries}

The above discussion shows that, although the orientation of $\hat{x}$ is arbitrary, not all allowed possibilities are equally convenient. 
...
For example, that was the case in going from Eq.~\eqref{eq:ellip} to Eq.~\eqref{eq:ellip_frame} above: if we know a priori that the waves from a given source will always be elliptically polarized, then it makes sense to construct our wave frame so that it always lies along the principal directions of the polarization ellipse (i.e., $\theta=0$).
This is the case for a nonprecessing compact binary coalescence (CBC).
...
In this convention, $\psi$ is identical to the source \emph{position angle} $\Psi$, as would be defined in regular astronomy.
...
Although this equivocation ($\psi = \Psi = \theta$) vastly simplifies analysis, it is useful to keep in mind that the three angles are conceptually distinct.
