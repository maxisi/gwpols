...
Unlike the linear modes of Eq.~\eqref{eq:lin}, however, the tensors of Eq.~\eqref{eq:circ} do not mix under rotations around the direction of propagation:
...
\end{align}
where the complex-valued Fourier amplitudes are defined by $\tilde{H}(\omega) \equiv (2\pi)^{-1} \int (h_+ - i h_\times) \exp(i\omega t)\, \infd t$, following our Fourier transform convention in Eq.~\eqref{eq:ft}.
...
We will make use of Jones vectors in computing Jacobians below.
...
%This is, of course, nothing more than another representation for the spectral mode amplitudes in Eq.
These Jones vectors are expressed in terms of their linear $+/\times$ projections, but they could be equally expressed in any other basis composed of two antipodal vectors defined on the (unit radius) Poincar\'{e} sphere of polarization states.
...
However, Stokes parameters have the advantage of being easily generalizable to unpolarized waves, which can be achieved by replacing the polarization amplitudes by their expected values (or time averages) in Eq.~\eqref{eq:stokes}; in that case, $Q=U=V=0$ and there is no Poincar\'{e} sphere to speak of.
...
We arrived at the generic form of a fully polarized mode in Eq.~\eqref{eq:hcomp_ellip} in the context of a fully generic Fourier decomposition of a plane wave, Eq.~\eqref{eq:hcomp_fd_rl}, where this arises naturally.
...
h_+ = \sum A_n(t) \hspace{-1pt} \left[\cos \Phi_n(t) \cos \theta_n - \epsilon_n \sin \Phi_n(t) \sin\theta_n \right] ,
\end{equation}
\begin{equation} \label{eq:ellip_sum_c}
h_\times = \sum A_n(t) \hspace{-2pt} \left[ \cos \Phi_n(t) \sin \theta_n + \epsilon_n \sin \Phi_n(t) \cos\theta_n \right] ,
\end{equation}
\end{subequations}
generalizing Eq.~\eqref{eq:hcomp_ellip} with a sum over some number of modes indexed by $n$ and (slowly) time varying amplitudes $A_n(t)$ and (quickly) time varying phases $\Phi_n(t)$, with some prescribed functional form.%
\footnote{The parameters $\epsilon_n$ and $\theta_n$ could also be made to vary adiabatically but that is usually not done in practice.}
...
If that is the case, then Eq.~\eqref{eq:spherical} is equivalent to the elliptical decomposition of Eq.~\eqref{eq:ellip_sum}.
