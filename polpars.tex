...
Implicitly, these polarization tensors are a function of the wave propagation direction (or, equivalently, the source location).
...
With absolute generality, the local value of the GW field in Eq.~\eqref{eq:hij} at any given point can be written as a decomposition over polarization modes $p$ in a plane-wave expansion,
...
Since $h_{ij}$ is real valued, the polarization Fourier amplitudes for any given propagation direction, $\tilde{h}_p(\omega, \hat{k}) \equiv (2\pi)^{-1} \int h_p(t, \hat{k})\, \infd t$, must  satisfy the complex-conjugate symmetry $\sum_p \tilde{h}_p(-\omega, \hat{k}) e^p_{ij} \equiv \left[\sum_p \tilde{h}_p(\omega, \hat{k}) e^p_{ij}\right]^*$; for the linear polarizations, this reduces to
...
If the mode is purely R-polarized, it will be given at the spatial origin ($z=0$) by
\begin{align}
...
Using this fact, an economic way of expressing the information in Eq.~\eqref{eq:planewave} for any given direction of propagation $\hat{k}$ is to write the time-domain complex strain at the spatial origin ($\vec{x}=0$) as an integral of the form
\begin{align} \label{eq:hcomp_fd}
h_+ - i h_\times = \int_{-\infty}^{+\infty} \tilde{H}(\omega)\, e^{-i \omega t} \,\infd \omega \, ,
\end{align}
where the complex-valued Fourier amplitudes are defined by $\tilde{H}(\omega) \equiv (2\pi)^{-1} \int (h_+ - i h_\times) \exp(i\omega t)\, \infd t$, and we are assuming an implicit dependence on $\hat{k}$.
...
This expression encapsulates all possible morphologies of a monochromatic GW---as special cases, $\epsilon = \pm 1$ encodes an R (L) circularly-polarized wave, while $\epsilon =0$ encodes a $+$ ($\times$) linearly-polarized wave if $\theta = 0,\pi$ ($\theta = \pm \pi/2$).
