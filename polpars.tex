...
\eeq
where $\hat{x}$ and $\hat{y}$ are orthonormal vectors that, with $\hat{z}$, form a right-handed orthonormal basis; we will call this the \emph{wave frame}.
...
With absolute generality, the local value of the GW field in Eq.~\eqref{eq:hij} at any given point can be represented as a decomposition over polarization modes in a plane-wave expansion,
...
These are constructed from the basis tensors
\beq
...
This means that, for a purely R-polarized wave, the cross polarization will lag behind the plus polarization by $\pi/2$ radians in phase.
Consequently, a purely R-polarized wave will deform a ring of freely-falling particles into an elliptical pattern that rotates counter-clockwise when looking towards the source, i.e., it follows the right-hand rule relative to the direction of propagation (pointing away from the source).
...
This is known as the source ``source based'' handedness convention.
...
If this quantity, the \emph{complex strain}, is purely real (imaginary) the wave is purely plus (cross) polarized.
...
Using this fact, an economic way to express the information in Eq.~\eqref{eq:planewave} for a fixed direction of propagation and at the spatial origin ($\vec{x}=0$) is to write the time-domain complex strain as the following integral:
\begin{align} \label{eq:hcomp_ft}
h_+ - i h_\times = \int_{-\infty}^{+\infty} \tilde{H}(\omega)\, e^{-i \omega t} \,\infd \omega \, ,
\end{align}
where the complex-valued Fourier amplitudes are defined by $\tilde{H}(\omega) \equiv \int (h_+ - i h_\times) \exp(i\omega t) \infd t$.
Unlike in Eq.~\eqref{eq:planewave}, these Fourier amplitudes clearly need not satisfy $\tilde{H}(-\omega) = \tilde{H}^*(\omega)$.
In fact, given the interpretation of $\exp(\pm i \omega t)$ discussed above, the positive (negative) frequency Fourier amplitudes in Eq.~\eqref{eq:hcomp_ft} must encode contributions from the R-polarized (L-polarized) portion of the waveform $\tilde{H}_R$ ($\tilde{H}_L$), so that we can rewrite Eq.~\eqref{eq:hcomp_ft} as
\begin{align}
h_+ - i h_\times = \int_{0}^{\infty}\left[ \tilde{H}_R(\omega)\, e^{-i \omega t} + \tilde{H}_L(\omega)\, e^{i \omega t}\right] \infd \omega .
\end{align}
If $\tilde{H}_R(\omega) = \tilde{H}_L(\omega)^*$, then it is indeed the case that $\tilde{H}(-\omega) = \tilde{H}(\omega)^*$ and we must have a purely plus-polarized wave.
...
% In fact, we could easily rewrite \eq{hij} in terms of the corresponding right and left handed polarizations, $h_{R/L} = h_+ \pm i h_\times$, which are invariant under $\psi$ rotations (concretely, eigenvectors of the helicity operator with eigenvalues $\pm 2$).
