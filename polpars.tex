...
\documentclass[aps,prd,twocolumn,superscriptaddress,preprintnumbers,floatfix,nofootinbib]{revtex4}
...
If the spins of the objects are a aligned and the source does not precess, one can show that, in that frame, $h_+$ and $h_\times$ for the dominant ($\ell=|m|=2$) moment of the radiation take the form
...
Equations \eqref{eq:cbc} describe an elliptically polarized wave.
This is a consequence of the geometry of our assumed source, which is symmetric under reflections over the orbital plane.
In full analogy to electromagnetic waves, a GW is elliptically polarized if there exists \emph{some} wave-frame orientation in which the two polarizations can be written as
...
In the case of an inspiral signal, our choice of $\hat{x}$ in Eq.~\eqref{eq:cbc} makes it immediately clear that this is an elliptically polarized wave with ellipticity
\beq
\epsilon = \frac{2 \cos\iota}{1 + \cos^2\iota}\, ,
\eeq
that is, just the ratio of plus and cross amplitudes in this frame.
...
Crucially, an elliptical wave will generally not take the form of Eq.~\eqref{eq:ellip_frame} for \emph{any} arbitrary choice of $\hat{x}$---only signals with $\epsilon=\pm1$ will take the form of Eq.~\eqref{eq:ellip_frame} irrespective of the wave frame orientation.
