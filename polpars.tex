...
\footnote{Since $\phi_{R/L}$ are $2\pi$-periodic, the most generic relation between them and $\theta$, $\phi$ is actually $\theta = [\phi_L - \phi_R  + 2\pi k]/2 \mod 2\pi$ and $\phi = [\phi_L + \phi_R  + 2\pi j]/2 \mod 2\pi$ for any integers $k,j$. \label{foot:angles}}
...
However, this results in a double covering of the phase space, since the waveform is invariant under the addition or subtraction of $\pi$ to $\theta$ and $\phi$, i.e., the transformations $\{\theta, \phi\} \to \{\theta \pm \pi, \phi \pm \pi\}$, as it is easy to infer from Fig.~\ref{fig:ellipse} (this degeneracy can be traced back to the property discussed in Footnote \ref{foot:angles} in relation to $\phi_{R/L}$).
This full range is required to allow for all waveform phases; however, fundamentally, $\theta$ would only enter the waveform as $2\theta$ had we defined the phase offset with respect to the $+$-polarization axis, i.e., replacing $\phi$ by $\hat{\phi} \equiv \theta + \phi = \phi_L$; with that replacement, $\theta$ need only vary over $[0, \pi]$ (or $[-\pi/2, \pi/2]$), as it is clear by considering the orientation of the ellipse in Fig.~\ref{fig:ellipse} irrespective of the initial phase. 
