...
\added[comment=$\uparrow$]{To understand the physical significance of the circular polarizations,}
consider a purely R-polarized monochromatic mode $h^R_{ij}$ with frequency $\omega > 0$, unit amplitude and zero phase offset; at the spatial origin ($\vec{x}=0$), the time-domain strain \added{tensor} corresponding to such a mode will be given by
...
\end{align}
using the definition from Eq.~\eqref{eq:circ}, \added{with the $\Re$ operator taking the real part to ensure a real-valued metric perturbation}.
...
\added[comment=FN]{This choice of prefactor can be motivated by noting that $\tilde{h}_{R/L}(\omega) = 2\pi \delta(\omega - \omega_0) C_{R/L}/2 = \delta(f - f_0) C_{R/L} / 2$ implies that $C_{R/L}/2$ are amplitude densities with respect to the frequency $f \equiv \omega/2\pi$; the additional factor of $1/2$ normalizes the signal power such that $h_+^2 + h_\times^2 = 1$ for $|C_R|=1, |C_L|=0$ or $|C_R|=0, |C_L|=1$.}
...
\added[comment=FN]{The effect of this traceless scalar polarizations is to simultaneously stretch (squeeze) along the $x$ and $y$ directions while squeezing (stretching) along the $z$ direction; the fully-trace scalar mode, to which current detectors are insensitive, stretch and squeeze space isotropically in all three directions.}
