...
The amplitude and ellipticity of each mode are determined by both (1) the intrinsic amplitudes $\mathcal{H}_{\ell \pm m}$, and (2) the viewing angle $(\iota, \varphi)$, through the ${}_{-2} Y_{\ell m}$ factors.
...
The strain for a given $(\ell, |m|)$ mode, as given by the summand in \eq{spherical_modes}, can be further rewritten as
\begin{equation}
...
\end{equation}
with the plus (minus) sign for the plus (cross) polarization (see Appendix B of \cite{Isi:2021iql}); here we have used the symmetry that ${}_{-2} Y_{\ell -m}(\iota,\varphi) = (-1)^{\ell+m} {}_{-2} Y_{\ell m}^*(\pi-\iota,\varphi+\pi)$ \cite{goldberg:1967}.
