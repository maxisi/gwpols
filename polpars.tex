...
\begin{abstract}
We review the formalism underlying the description of gravitational wave (GW) polarizations and the coordinate frames used to define them.
In the process, we clarify the notion of ``polarization angle'' and identify three conceptually distinct definitions.
We describe how those are related and how they arise in the practice of GW data analysis, explaining in detail the associated reference frames and conventions that have become standard in LIGO-Virgo data analysis.
Furthermore, we show that any GW signal can be expressed as a superposition of elliptical (i.e., fully-polarized) states, and describe in detail the properties and possible parametrizations of such elementary states.
We discuss a variety of parametrizations in use in the literature, and compute Jacobians for the coordinate transformations relating them.
...
Gravitational waves (GWs) come in two distinct polarization states, whose time evolution encodes information about the structure of \ac{GR} and the dynamics of the source.
...
When analyzing data from detectors like LIGO \cite{TheLIGOScientific:2014jea} and Virgo \cite{TheVirgo:2014hva}, it is natural to parametrize these polarizations differently depending on the application.
For instance, searches for \acp{CBC} aim to relate the signal observed by different detectors to templates obtained from theory, and thus benefit from describing GW polarizations in the frame used to produce the predictions.
On the other hand, unmodeled (or semimodeled) analyses aim to reconstruct GWs without relying on specific input from theory, and must instead make an arbitrary choice in orienting the polarization frame.
Furthermore, lacking waveform templates, unmodeled analyses must also decide how to parametrize the GW polarization state and its time evolution in a way sufficiently flexible to capture a range of morphologies but parsimonious enough to remain computationally tractable.
...
The abundance of definitions and parametrizations of GW polarizations extends from the descriptions in the literature to the implementations of actual data analysis pipelines.
Such variety can not only be conceptually confusing, but can also hinder comparisons across analyses with different conventions, or even obfuscate the interpretation of the output from a given analysis.
As an example, these considerations come into play when parametrizing continuous GWs from galactic pulsars, and in relating such (projected) measurements to electromagnetic observations of the source orientation \cite{Isi:2017equ,Pitkin:2017qfy}.
They are relevant in parametrizing ringdown signals for black hole spectroscopy, where multiple factorizations are possible for the polarization amplitudes \cite{Isi:2021iql}.
They are also important for understanding the implications of different treatments of polarization ellipticity in unmodeled analyses, e.g., with the \textsc{BayesWave} algorithm \cite{Cornish:2014kda,Cornish:2020dwh,Chatziioannou:2021mij}, and in comparing such results to those from modeled \ac{CBC} analyses.

This paper provides a comprehensive exposition of the formalism underlying GW polarizations in practical applications.
The goal is twofold:
...
\emph{second}, technical, in explicitly working out the coordinate transformations that link different parametrizations, and providing ready-to-use expressions for the corresponding Jacobians---the mathematical factors that translate between posterior probability densities obtained under different parametrizations, which are required to exchange priors in parameter estimation codes or similar applications.
%
%It is important to understand the priors implied by the choice of parametrization through the corresponding Jacobians.
%
%
In this work, the exposition is geared towards observers, or theorists interested in drawing connections to observation---as such, it strives for concreteness over abstraction, and, in particular, steers away from formal connections between the treatment of GW polarizations and the mathematical structure of GR.
...
Having established the importance of elliptical (fully polarized) modes, Sec.~\ref{sec:ellip} examines their key properties, outlines some of their uses, and sketches their connection to spin-weighted spherical harmonics.
...
Finally, Sec.~\ref{sec:nongr} briefly covers generalizations of these ideas to beyond-GR polarization states.
...
\end{pmatrix} ,
\eeq
where the plus ($+$) and cross ($\times$) polarization functions, $h_{+/\times}$, depend implicitly on the retarded time, $t - R/c$, in a way determined by the source dynamics and by the (luminosity) distance $R$ to the source, as well as on any other relevant source parameters controlling the amplitude and phase of the wave, as dictacted by Einstein's equations.
...
Arguing for the conceptual importance of elliptical (i.e., fully-polarized) modes, we outlined several of their key properties and reviewed a number of mathematical tools (Jones vectors, Poincar\'{e} sphere, Stokes parameters) useful in their description.
Since a large number of signal morphologies can be captured by superpositions of fully-polarized states, we emphasized their practical importance for GW data analyiss in unmodeled (or loosely modeled) applications, as well as in connection to spin-weighted spherical harmonic decompositions of the GW strain from planar sources (e.g., nonprecessing \acp{CBC}).

We then clarified the conceptual differences between different notions of ``polarization angle,'' and showed how they can often (but not always) be used interchangeably in practice.
...
To understand the connections between practical applications that make use of different such polarization parametrizations, we computed Jacobians for the corresponding coordinate transformations.
This allows us to understand the implications of parametrizations for the polarization of a signal assumed by, e.g., \textsc{BayesWave} or ringdown analyses.
We found that parametrizing the GW signal in terms of the circular polarization amplitudes, the linear polarization amplitudes, or the cosine and sine quadratures of the linear polarizations leads to implicitly favoring overall high signal intensities.
The parametrizations in terms of the linear amplitudes or their quadratures, as well as a parametrization in terms of an ellipse shape angle $\chi$, lead to favoring linear polarizations.
This preference is particularly pronounced for the parametrization in terms of the linear amplitudes, which also picks a preferred direction for the polarization ellipse, aligned with the plus and cross axes as determined by the implicit definition of the physical waveframe.

In the last section, we briefly touched on the generalization to metric theories of gravity without tensor modes, and how the tools examined in this work would generalize beyond GR.
