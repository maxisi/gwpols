...
As in a Fourier expansion, each phasing function will usually correspond to some given frequency $\omega_i$, so that $\Phi_i = \omega_i t + \phi_i$, while the $A_i(t)$ encodes some amplitude envelope evolving slowly over some timescale $\tau_i \equiv 1/\gamma_i$.
...
Moreover, even if there was a choice of waveframe in which the two linear polarizations looked completely dissimilar, the polarizations will look spectrally similar to generic observers whose frame is oriented otherwise.
...
We can then refactor Eq.~\eqref{eq:spherical} so that we only sum over positive values of $m$,
\begin{widetext}
...
The amplitude and ellipticity of each mode are determined by both the intrinsic amplitudes $C_{\ell \pm n}'$ and the viewing angle through the ${}_{-2} Y_{\ell m}$ factors.



\section{Coordinate transformations}

Although visually useful, the elliptical parameterization in terms of $\{A, \epsilon, \theta, \phi\}$ is not always ideal for sampling purposes due to degeneracies intrinsic to those parameters.
...
\end{align}
then note that setting $A_L = 0$ ($A_R =0$) results in a fully right-handed (left-handed) circularly polarized wave.
This is equivalent to Eq.~\eqref{eq:ellip_sum} if we set
\begin{equation}
\begin{cases}
A = A_R + A_L \\
...
To see this, first note that these equations can be equivalently rewritten as
\begin{subequations}
\begin{equation}
...
\end{equation}
\end{subequations}
where $A_{+/\times}$ could potentially subsume any (slow) time dependence present implied by $A(t)$ in Eq.~\eqref{eq:ellip}.
Equivalence with 
\begin{equation}
\begin{cases}
...
\phi_R = \mathrm{atan2}\left(A_{+,s} -A_{\times,c}, A_{+,c} + A_{\times,s} \right) \\
\phi_L = \mathrm{atan2}\left(-A_{+,s} -A_{\times,c}, A_{+,c} - A_{\times,s} \right)\, 
\end{cases}
\end{equation}
where, to simplify the notation, we have defined the cosine and sine quadratures
\begin{subequations}
\begin{equation}
A_{+/\times, c} \equiv A_{+/\times} \cos \phi_{+/\times} \, ,
\end{equation}
\begin{equation}
A_{+/\times, s} \equiv A_{+/\times} \sin \phi_{+/\times} \, .
\end{equation}
\end{subequations}
Together with the transformation from the previous section, we can then get $\{A, \epsilon, \theta, \phi\}$ as a function of $\{A_+, A_\times, \phi_+, \phi_\times\}$.

The Jacobian between the two parametrizations is
\begin{equation}
J = \frac{1}{2 A} \sqrt{\left(\frac{1 + \epsilon^2}{1 - \epsilon^2}\right)^2 - \cos^2 2\theta}
\end{equation}
