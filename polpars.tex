...
The requirement for $\theta$ to extend all the way up to $2\pi$ (or $\pm \pi$) arises from our definition of the phase angle $\phi$ with respect to the semimajor axis of the ellipse (see Fig.~\ref{fig:ellipse}).
Fundamentally, however, $\theta$ need only be specified over half that range in order to determine the orientation of the ellipse, disregarding the signal phase.
Indeed, had we instead chosen to work in terms of a phase angle $\bar{\phi} \equiv \theta - \phi = -\phi_R$ defined relative to the $h_+$ axis (and thus decoupled from $\theta$), \eq{hcomp_ellip} would have become
...
Based on \eq{hcomp_ellip}, the angle $\theta$ is required to vary over the full range $[0, 2\pi]$ in order to allow for all waveform phases; however, fundamentally, $\theta$ would only enter the waveform as $2\theta$ had we defined the phase offset with respect to the $+$-polarization axis, i.e., replacing $\phi$ by $\hat{\phi} \equiv \theta + \phi = \phi_L$; with that replacement, $\theta$ need only vary over $[0, \pi]$ (or $[-\pi/2, \pi/2]$), as it is clear by considering the orientation of the ellipse in Fig.~\ref{fig:ellipse} irrespective of the initial phase. 
