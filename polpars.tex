...
Gravitational waves (GWs) come in two distinct polarization states, which encode information about the structure of \ac{GR} and the dynamics of the source.
...
When analyzing data from detectors like LIGO \cite{TheLIGOScientific:2014jea} and Virgo \cite{TheVirgo:2014hva}, it is necessary (or convenient) to parametrize these polarizations differently depending on the application.
For instance, searches for \acp{CBC} aim to relate the signal observed by different detectors to templates obtained from theory, and thus benefit from describing GW polarizations in the same frame used to produce the predictions.
...
The abundance of conventions for the definition and parametrization of GW polarizations extends from the descriptions in the literature to the implementations of actual data analysis pipelines.
Such variety can not only be conceptually confusing, but can also hinder comparisons across analyses with different conventions, or even obfuscate the interpretation of a given analysis.
As an example, these considerations come into play when parametrizing continuous GWs from galactic pulsars, and in relating such expected measurements to electromagnetic observations of the source orientation \cite{Isi:2017equ,Pitkin:2017qfy}.
They are also relevant in parametrizing ringdown signals for black hole spectroscopy, where multiple factorizations are possible for the polarization amplitudes \cite{Isi:2021iql}.
They are also important for understanding the implications of different treatments of signal polarization in unmodeled analyses, e.g., with the \textsc{BayesWave} algorithm \cite{Cornish:2014kda,Cornish:2020dwh,Chatziioannou:2021mij}.

This paper attempts to bridge the different treatments in the literature by providing a comprehensive exposition of the formalism underlying GW polarizations and the different conventions used in parametrizing them.
The goal is twofold:
first, pedagogical, in reviewing different polarization conventions and their connections;
