...
\begin{figure}
\includegraphics[width=0.8\columnwidth]{diagram_ellipse.pdf}
...
This again reveals that our choice of coordinates was a good one: because this wave-frame orientation preserves the symmetries of the source, it also happens to be aligned with the principal directions of the polarization ellipse.
%
...
Having specified $h_+$ and $h_\times$ in that standard, source-based frame, all we need to do to predict the signal at a given detector is to evaluate \eq{h} by expressing the components of $h_{ij}$ and $D_{ij}$ in a common basis.
For ground-based detectors, this basis is usually the equatorial celestial coordinates introduced in Sec.~\ref{sec:pol}, namely $\{\hat{x},\hat{y},\hat{z}\}$ where $\hat{x}$ is parallel to the celestial equator pointing due West and, as always, $\hat{z}=\hat{k}$.
We can relate this absolute reference to any other valid choice of waveframe $\{\hat{x}', \hat{y}', \hat{z}'=\hat{z}\}$ by specifying the angle $\psi$ between $\hat{x}'$ and the celestial equator, following the right-hand rule around $\hat{z}$ \cite{LALSuite:wave}.
Knowing that the waveframe $\hat{y}$ is tied to the line of nodes, then $\psi = 0$ must mean that the line of nodes points towards the celestial North, or, equivalently, the projection of the orbital angular onto the plane of the sky is parallel to the celestial equator due West (see Fig.~\ref{fig:diagram_sourceframe}).
...
% Having specified $h_+$ and $h_\times$ in that standard, source-based frame, we need to determine how that frame itself is oriented with respect to the detectors in order to evaluate \eq{h}.
...
%In LIGO, it is conventional to compute the polarizations in a frame such that $\hat{y}$ lies along the intersection of the orbital plane with the plane of the sky (the line of nodes), $\hat{z}$ is parallel to the wave vector as required by \eq{hij}, and $\hat{x}$ completes the triad \cite{LALSuite:source}.

In this convention, $\psi$ is equivalent to the \emph{position angle} of the source's orbital angular momentum $\Psi$, defined to be angle between the projected orbital angular momentum and the celestial equator in the plane of the sky (following the right-hand rule relative to $\hat{z}$).
More generally, $\Psi = \psi - \Omega$ in terms of the longitude of the ascending node $\Omega$ with $\hat{x}$ as the origin of longitude ($\Omega = \pi/2$ in the default LIGO-Virgo convention) \cite{LALSuite:source}.
...
The LIGO-Virgo convention sets $\Omega=\pi/2$, so that $\hat{y}'$ is the ascending node and $\hat{x}'$ lies along the projection of $\hat{L}$ onto the plane of the sky ($\hat{L}'$).
}
...
Nonetheless, it is still conventional to tie $\hat{y}$ to the source, referring to the line of nodes as oriented at some specific point in the binary evolution (e.g., when the detected GW signal reaches 20 Hz).
...
In unmodeled analyses, it is not possible or useful to explicitly tie the polarization frame to properties of the source, since these analyses are not tailored to any specific source to begin with (or they purposely disregard source orientation information).
...
This indicates that a uniform prior on $\hat{A}$ and $\chi$ imposes a distribution that favors linear polarizations ($\epsilon = 0$) over circular ones ($\epsilon = \pm 1$), with probability density $\propto 1/J_0$. }
...
An elliptical wave, Eq.~\eqref{eq:hcomp_ellip}, can be specified in terms of the circular polarization basis elements as in \eq{ellip_circ},
where the $C_{R/L} \equiv A_{R/L} \exp(i\phi_{R/L})$ quantites control the amplitude and phase of the circularly-polarized components of the signal.
%
The representation in terms circular-mode amplitudes and phases is equivalent to Eq.~\eqref{eq:hcomp_ellip} if we impose
...
\end{equation}
with a proportionality constant of $1/\sqrt{2}$, which can be ignored in most applications.
...
Rather than using the circular basis, we could instead work with the linear polarization modes as the fundamental quantity and parametrize them as
...
The above equations also represent an elliptically polarized mode.
To relate this parametrization to that in Eq.~\eqref{eq:hcomp_ellip}, it is convenient to first map Eq.~\eqref{eq:Aphi} into the circular-basis parameters of the previous section through the transformation
...
$h_R = (h_+ - i h_\times)/\sqrt{2}$.
\emph{Top:} Following Eq.~\eqref{eq:Aphi}, the plus and cross Jones vectors, $\vec{C}_{+/\times} = A_{+/\times} \exp(i \phi_{+/\times})$, subtend angles $\phi_{+/\times}$ relative to the real axis ($x$, abscissa) and thus have Cartesian coordinates $\left(\xpc, \ypc \right)$, in terms of the quadratures defined in Eq.~\eqref{eq:xy}; the vector $i\vec{C}_\times$ is orthogonal to $\vec{C}_\times$, so that it subtends an angle $\pi/2 + \phi_\times$ with respect to the positive $x$-axis.
\emph{Bottom:} the right-handed Jones vector, $\vec{C}_R = \left(\vec{C}_+ + i \vec{C}_\times\right)/\sqrt{2}$, has components $\left(\xp - \yc, \yp + \xc\right)/\sqrt{2}$ and, therefore, $\phi_R = \mathrm{atan2}( \yp + \xc, \xp - \yc)$; since the acute angle between $\vec{C}_+$ and $i\vec{C}_\times$ is $\pi - (\pi /2 + \phi_\times - \phi_+) = \phi_+ - \phi_\times + \pi/2$, the law of cosine implies $2 A_R^2 = A_+^2 + A_\times^2 - 2 A_+ A_\times \sin(\phi_\times - \phi_+)$.
This explains Eq.~\eqref{eq:Aphi_to_Cphi} for the right-handed case (the left-handed one is analogous).
}
\label{fig:diagram_apac}
\end{figure}

We can gain some intuition for Eq.~\eqref{eq:Aphi_to_Cphi} by considering the relationship between the phasors associated with either the linear or circular polarizations (Fig.~\ref{fig:diagram_apac}).
...
Equation~\eqref{eq:Aphi_to_Cphi} then follows trivially from the law of cosines, as illustrated in Fig.~\ref{fig:diagram_apac}.
