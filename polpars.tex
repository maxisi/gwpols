...
Since $\hat{A} = A \sqrt{1+\epsilon^2}$ and $\max(\epsilon)= 1$, we should make sure to bound $A \leq  A_{\max} \equiv \hat{A}_{\max} /\sqrt{2}$ (dashed line), as in the example of Fig.~\ref{fig:jac_example}.
...
An isotropic prior in $\phi_R$ and $\phi_L$ is also isotropic in $\theta$ and $\phi$.
...
Without enforcing a $A \leq A_{\max}$ constraint, the ellipticity distribution will no longer be uniform, instead favoring linear polarizations (left panel of Fig.~\ref{fig:jac_rl}).
...
Restricting to $A < A_{\max}$, the density is proportional to $A$ and flat in $\epsilon$, following $\propto 1/J_1$ for the Jacobian in \eq{jac_Aeps_Arl} (orange curves).
}
\label{fig:jac_rl}
\end{figure}
